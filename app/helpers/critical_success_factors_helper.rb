module CriticalSuccessFactorsHelper
  def fillrightleftcriticalactionline(f)
    if @critical_success_factor.new_record?
      sql = "SELECT priority_program_action_lines.name, priority_program_action_lines.id  FROM priority_program_action_lines order by priority_program_action_lines.name"
   ...
   WHERE priority_program_action_lines_programs.program_id =" + :program_id.to_s + ") order by priority_program_action_lines.name"
